"""
Direct RB analysis class.
Direct RB is ran on a single qubit.
Target state is either 0 or 1. It is specified in each circuit's metadata.
"""
...
class RBAnalysisWithFlippedTargetStates(curve.CurveAnalysis):
    ...
    def _default_options(cls):
        ...
        default_options.data_subfit_map = {
             "target=0": {"target_state": 0},
             "target=1": {"target_state": 1}
        }
    ...
    def _generate_fit_guesses(
            ...
            """
            target_0_data = curve_data.get_subset_of("target=0")
            target_1_data = curve_data.get_subset_of("target=1")

            user_opt.bounds.set_if_empty(
                a0=(0, 1),
                a1=(-1, 0),  # note the opposite sign
                alpha=(0, 1),
                b=(0, 1),
            )

            # compute the guesses from the original (target=0 case) data
            b_guess = 1 / 2
            alpha_guess = curve.guess.rb_decay(target_0_data.x, target_0_data.y, b=b_guess)
            a_guess = (target_0_data.y[0] - b_guess) / (alpha_guess ** target_0_data.x[0])
